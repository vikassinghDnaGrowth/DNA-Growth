...
const TeamCard = React.memo(({ member }) => {
  ...
  const shortDescription = useMemo(() => description.substring(0, 100) + "...", [description]);
  ...
  return (
    <div className="border-2 border-gray-700 sm:rounded-full rounded-3xl hover:text-white text-black flex flex-col items-center py-10 w-10/12 lg:w-10/12 md:w-8/12 hover:bg-[rgb(22,103,161)] transition-all duration-700 relative">
      <div className="flex justify-center mb-4 absolute sm:static -top-[16%] sm:-top-0">
        <img src={avatar} alt={`${name}'s avatar`} className="w-1/4 md:w-9/12 rounded-full" />
      </div>
      ...
      <button onClick={openModal} className="mt-2 underline text-xs md:text-sm text-customBlue">
        Read More
      </button>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50" onClick={closeModal}>
          <div className="bg-white rounded-lg w-10/12 md:w-1/3 p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-center">
              <h2 className="text-2xl font-semibold text-customBlue">{name}</h2>
              <button onClick={closeModal} className="text-lg font-bold text-white" aria-label="Close modal">
                X
              </button>
            </div>
